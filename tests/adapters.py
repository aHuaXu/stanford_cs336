...
def run_linear(
    ...
    linear_layer = LinearLayer(d_in, d_out, device=weights.device, dtype=weights.dtype)
...
def run_embedding(
    ...
    embedding_layer = EmbeddingLayer(vocab_size, d_model, device=weights.device, dtype=weights.dtype)
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    multihead_attention_layer = MultiHeadAttention(
    ...
    multihead_attention_layer.load_state_dict(state_dict)
    return multihead_attention_layer(in_features, token_positions=token_positions)
...
def run_rope(
    ...
    """
    rope_layer = RotaryPositionalEmbedding(theta, d_k, max_seq_len, device=in_query_or_key.device)
